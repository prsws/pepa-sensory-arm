"""Custom exceptions for the Pepa Sensory Arm component.

This module defines all custom exceptions used throughout the Pepa Sensory Arm
...
class PepaSensoryArmError(Exception):
    """Base exception for all Pepa Sensory Arm errors.

    This is the parent exception class that all other Pepa Sensory Arm exceptions
    inherit from. Use this for catching any Pepa Sensory Arm-related error.

    Example:
        try:
            # Some Pepa Sensory Arm operation
            pass
        except PepaSensoryArmError as error:
            _LOGGER.error("Pepa Sensory Arm error: %s", error)
    """


class ContextInjectionError(PepaSensoryArmError):
...
class ToolExecutionError(PepaSensoryArmError):
...
class AuthenticationError(PepaSensoryArmError):
...
class TokenLimitExceeded(PepaSensoryArmError):
...
class RateLimitExceeded(PepaSensoryArmError):
...
class PermissionDenied(PepaSensoryArmError):
...
class ValidationError(PepaSensoryArmError):
...
class EntityNotFoundError(PepaSensoryArmError):
...
class ServiceUnavailableError(PepaSensoryArmError):
