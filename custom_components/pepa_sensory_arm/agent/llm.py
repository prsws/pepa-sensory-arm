"""LLM API communication mixin for PepaSensoryArm.

This module provides the LLMMixin class that handles all synchronous LLM API
communication for the PepaSensoryArm. It manages HTTP sessions, constructs API requests
in OpenAI-compatible format, and processes responses.

Architecture:
    LLMMixin is designed as a mixin class to be inherited by PepaSensoryArm. It provides
...
Usage Example:
    The mixin is used through inheritance in PepaSensoryArm:

        class PepaSensoryArm(LLMMixin, StreamingMixin, MemoryExtractionMixin):
...
from ..exceptions import AuthenticationError, PepaSensoryArmError
from ..helpers import (
    build_api_url,
    build_auth_headers,
    is_ollama_backend,
...
class LLMMixin:
    ...
    async def _call_llm(
        ...
        Raises:
            ...
            PepaSensoryArmError: If API call fails
        """
        ...
        async def make_llm_request() -> dict[str, Any]:
            ...
            try:
                ...
                raise PepaSensoryArmError(f"Failed to connect to LLM API: {err}") from err
