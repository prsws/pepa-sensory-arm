"""Core conversation agent implementation for Pepa Sensory Arm.

This module implements the PepaSensoryArm class, which serves as the central orchestrator
for all conversation-related functionality in the Pepa Sensory Arm integration. It brings
...
Architecture:
    The PepaSensoryArm class uses a mixin-based architecture to separate concerns:
...
Key Classes:
    PepaSensoryArm: Main conversation agent that integrates with Home Assistant's
...
class PepaSensoryArm(
    LLMMixin,
    StreamingMixin,
    MemoryExtractionMixin,
    AbstractConversationAgent,
):
    """Main conversation agent that orchestrates all Pepa Sensory Arm components.
    ...
    def __init__(
        ...
        """Initialize the Pepa Sensory Arm.
    ...
    async def process_message(
        ...
        Raises:
            PepaSensoryArmError: If processing fails
        """
        # Ensure tools are registered (lazy initialization)
        self._ensure_tools_registered()
    ...
    async def _async_process_streaming(
        ...
        if entry_id is None:
            _LOGGER.warning("Could not find entry_id for streaming, using 'pepa_sensory_arm'")
            entry_id = "pepa_sensory_arm"
